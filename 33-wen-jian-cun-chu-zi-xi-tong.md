...
文件与存储子系统管理设备的文件系统、应用沙箱目录与媒体库，并支持分布式文件访问。它与[23-权限与访问控制](23-quan-xian-guan-li-zi-xi-tong.md) 的沙箱隔离、[17-分布式数据管理](17-fen-bu-shi-shu-ju-guan-li.md) 的分布式能力协同，决定了应用"能读到什么、写到哪里"。

## 一、文件系统

- **本地文件系统**：标准目录布局（`/system`、`/vendor`、`/data`、`/storage` 等），由 [19-内核子系统](19-nei-he-zi-xi-tong-linux-nei-he-jia-gou.md) 的文件系统驱动支撑。
- **分布式文件系统（hmdfs）**：把远端可信设备的目录挂载到本地命名空间，应用访问远端文件如同访问本地——前提是设备已通过 [23-安全子系统](23-an-quan-zi-xi-tong.md) 认证。
...
图片、视频、音频等媒体资源由媒体库统一管理，应用通过选择器 / 媒体管理 API 进行增删查改，且受 [23-权限与访问控制](23-quan-xian-guan-li-zi-xi-tong.md) 的媒体权限约束。
...
3. **分布式文件打不开**：目标设备未认证或离线，先完成 [27-分布式任务调度](27-fen-bu-shi-ren-wu-diao-du.md) 的设备信任流程。
