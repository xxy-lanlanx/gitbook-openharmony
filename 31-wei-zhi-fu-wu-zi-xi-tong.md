...
位置服务子系统提供设备定位能力，融合 GNSS、基站与 Wi-Fi 定位，并支持地理编码与地理围栏。它是地图、出行、本地生活类应用的基础，同时受到严格隐私约束——强依赖 [23-权限与访问控制](23-quan-xian-guan-li-zi-xi-tong.md) 的位置权限，并借助 [30-通信子系统](30-tong-xin-zi-xi-tong.md) 的 Wi-Fi / 基站辅助提升精度。
...
- **权限类型**：位置属于 `user_grant` 敏感权限，必须在运行时申请（见 [23-权限与访问控制](23-quan-xian-guan-li-zi-xi-tong.md)）；后台定位还需 `ohos.permission.LOCATION_IN_BACKGROUND`。
