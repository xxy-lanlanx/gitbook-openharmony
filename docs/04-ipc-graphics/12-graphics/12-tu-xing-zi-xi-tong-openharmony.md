...
| 维度 | 轻量系统图形 UI | 标准系统 ArkUI |
|------|-----------------|----------------|
| 开发语言 | C++ | ArkTS（声明式） |
| 控件风格 | 命令式（创建、设置、添加到父容器） | 声明式（描述 UI 结构） |
| 动画 | 属性动画 + 自定义插值 | 内置动画 + 过渡效果 |
| 渲染 | 2D 图形库 / 可选硬件加速 | Rosen 渲染引擎 / GPU 加速 |
| 内存占用 | 极小（KB 级） | 较大（MB 级） |
| 适用设备 | MCU、传感器、面板 | 手机、平板、车机 |
...
* **内存管理**：轻量系统内存极为有限，图形子系统采用对象池、预分配缓冲等策略减少动态分配；图片解码后按目标尺寸缩放，避免大图占用过多内存。
* **主题与皮肤**：支持通过替换颜色、字体、图片资源包实现主题切换，资源编译后按限定词加载（参见[32-全球化与国际化](../32-quan-qiu-hua-zi-xi-tong.md)）。
