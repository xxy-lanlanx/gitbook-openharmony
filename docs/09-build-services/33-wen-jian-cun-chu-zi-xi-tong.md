...
| 目录 | 用途 | 典型内容 | 掉电是否保留 |
|------|------|----------|--------------|
| `/system` | 系统镜像 | 系统库、框架、预置应用 | 是（只读） |
| `/vendor` | 厂商定制 | 芯片驱动、厂商 HAL | 是（只读） |
| `/data` | 用户数据 | 应用沙箱、用户配置、日志 | 是 |
| `/storage` | 外部存储 | 媒体文件、下载、文档 | 是 |
| `/cache` | 临时缓存 | 应用缓存、下载中间文件 | 否（系统可清理） |
...
OpenHarmony 标准系统支持基于文件的加密（FBE，File-Based Encryption）：

- **EL1**：设备启动后即可访问，用于系统与应用基础数据；
- **EL2**：用户首次解锁后才解密，用于用户敏感数据（如照片、文档）。
- 密钥由[23-安全子系统](../../07-security/23-an-quan-zi-xi-tong.md) 的 HUKS 管理，绑定用户凭证与设备可信状态。
...
- **文件选择器（FilePicker）**：系统提供的统一文件选择 UI，允许用户跨应用选择文件，应用获得的是临时授权路径，无需直接访问他人沙箱。
- **分享（Share）**：通过 UDMF 与系统分享通道，应用可以将文件以标准化数据结构分享给其他应用，而不暴露真实路径。
...
hmdfs（Harmony Distributed File System）是 OpenHarmony 为分布式场景设计的文件系统：

- **透明挂载**：远端可信设备的目录挂载到本地 `/mnt/hmdfs/` 下，应用通过标准文件 API 访问；
- **按需拉取**：文件首次访问时从远端下载，本地缓存，后续访问直接读本地；
- **一致性**：远端文件修改后，通过事件通知本地缓存失效，下次访问重新拉取；
- **安全前提**：仅对已通过设备认证的同帐号设备开放， strangers 无法挂载。

## 七、常见问题与排查

1. **读取他人文件失败**：沙箱隔离；改用公共目录 / 媒体库 / 文件选择器等受控通道。
...
5. **存储空间不足**：检查 `cacheDir` 是否过大，及时清理；或使用 `storageStatistics` 查看各应用占用。
