...
- **轻量事件**：同进程内可用 `context.eventHub` 或 [29-通知与公共事件](../../09-build-services/29-tong-zhi-gong-gong-shi-jian.md) 的 `Emitter` 做线程/Ability 间通知。
...
结合[27-分布式任务调度](../../05-media-distributed/27-fen-bu-shi-ren-wu-diao-du.md)，Stage 模型可在 Want 中填入 `deviceId` 实现跨设备启动：
...
要点：目标设备必须经过[23-安全子系统](../../07-security/23-an-quan-zi-xi-tong.md) 的设备互信认证，且应用在两台设备上均已安装、权限齐备，否则调用会被拒绝。
...
- [07-NAPI接口](../../02-framework/07-napi-jie-kou.md)
